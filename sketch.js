...
function getTargetPosition() {
  let anchor = document.getElementById('particle-target');
  if (anchor) {
    let rect = anchor.getBoundingClientRect();
    return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
  }
  // Fallback: right-center of viewport
  return { x: windowWidth * 0.72, y: windowHeight / 2 };
}
...
function generateParticles(){
  ...
  for(let i = 0; i < totalPoints; i++){
    ...
    particles.push({
      x: random(width),
      y: random(height),
      tx: target.x - (img && img.width > 0 ? img.width / 2 : 100) + p.x,
      ty: target.y - (img && img.height > 0 ? img.height / 2 : 125) + p.y,
      darkness: p.darkness,
      vx: random(-1, 1),
      vy: random(-1, 1)
    });
...
function draw(){
  ...
  let distanceToMouse = dist(mouseX, mouseY, target.x, target.y);
  ...
  for(let p of particles){
    if(mouseForce > 0.05){
      ...
      p.x = lerp(p.x, target.x + (p.tx - target.x) * breathe, speed);
      p.y = lerp(p.y, target.y + (p.ty - target.y) * breathe, speed);
    } else {
      p.x += p.vx;
      p.y += p.vy;
